...
namespace Router
{
    void EnterpriseController::getEnterpriseInfo(const crow::request &request, crow::response &response)
    {
        try
        {
            ...
            {
                response.code = 404;
                response.write(crow::json::wvalue{{"message", "企业不存在"}}.dump());
                response.end();
                return;
            }
    ...
    void EnterpriseController::updateEnterpriseInfo(const crow::request &request, crow::response &response)
    {
        try
        {
            ...
            {
                response.code = 400;
                response.write(crow::json::wvalue{{"message", "更新企业信息失败"}}.dump());
                response.end();
                return;
            }

            response.code = 200;
            response.write(crow::json::wvalue{{"message", "企业信息更新成功"}}.dump());
            response.end();
        } catch (const std::exception &e)
        {
    ...
    void EnterpriseController::postNewJob(const crow::request &request, crow::response &response)
    {
        try
        {
            ...
            {
                response.code = 400;
                response.write(crow::json::wvalue{{"message", "创建职位失败"}}.dump());
                response.end();
                return;
            }

            response.code = 200;
            response.write(crow::json::wvalue{{"message", "职位创建成功"}}.dump());
            response.end();
        } catch (const std::exception &e)
        {
    ...
    void EnterpriseController::updateJob(const crow::request &request, crow::response &response, int jobID)
    {
        try
        {
            ...
            // 构建职位更新对象
            Model::JobPosting jobData;
            jobData.EnterpriseID = std::stoll(enterpriseId);// This seems to imply the job must belong to the enterprise, but it's not explicitly checked here against the jobID's original enterprise ID before update.
            ...
            {
                response.code = 400;
                response.write(crow::json::wvalue{{"message", "更新职位失败"}}.dump());
                response.end();
                return;
            }

            response.code = 200;
            response.write(crow::json::wvalue{{"message", "职位更新成功"}}.dump());
            response.end();
        } catch (const std::exception &e)
        {
    ...
    void EnterpriseController::deleteJob(const crow::request &request, crow::response &response, int jobID)
    {
        try
        {
            ...
            // 获取路径参数中的职位ID
            std::string jobId = std::to_string(jobID);
            if (jobId.empty())// Similar note as above for std::to_string
            {
                response.code = 400;
                response.write(crow::json::wvalue{{"message", "缺少职位ID参数"}}.dump());
                response.end();
                return;
            }
            ...
            {
                response.code = 400;
                response.write(crow::json::wvalue{{"message", "删除职位失败"}}.dump());
                response.end();
                return;
            }

            response.code = 200;
            response.write(crow::json::wvalue{{"message", "职位删除成功"}}.dump());
            response.end();
        } catch (const std::exception &e)
        {
    ...
    void EnterpriseController::updateApplicationStatus(const crow::request &request, crow::response &response, int aid)
    {
        try
        {
            ...
            response.code = 200;
            response.write(crow::json::wvalue{{"message", "申请状态更新成功"}}.dump());
            response.end();
        } catch (const std::exception &e)
        {
    ...
    void EnterpriseController::getPostedJobDetail(const crow::request &request, crow::response &response, int jobID)
    {
        try
        {
            ...
            {
                response.code = 404;
                response.write(crow::json::wvalue{{"message", "职位不存在"}}.dump());
                response.end();
                return;
            }
            ...
            {
                response.code = 403;
                response.write(crow::json::wvalue{{"message", "无权访问此职位"}}.dump());
                response.end();
                return;
            }
